...
const Page = ({
  ...
  useEffect(() => {
    const fn = async () => {
      try {
        ...
        const totalBonusPaid = finalSettlementData?.bonusDetails?.reduce(
          (acc, e) => (e.status ? acc + e.bonus : acc),
          0
        );
  ...
  ///////////////////////////////////////////////
  console.log(
    'sidha total attendance per year',
    finalSettlementData?.totalAttendancePerYear
  );

  const [reversedTotalAttendancePerYear, setReversedTotalAttendancePerYear] =
    useState([]);

  useEffect(() => {
    const temp = [];
    if (finalSettlementData?.totalAttendancePerYear) {
      for (
        let i = 0;
        i < finalSettlementData?.totalAttendancePerYear?.length;
        i++
      ) {
        const item =
          finalSettlementData?.totalAttendancePerYear[
            finalSettlementData?.totalAttendancePerYear?.length - i - 1
          ];
        temp.push(item);
      }
    }
    setReversedTotalAttendancePerYear(temp);
  }, [finalSettlementData?.totalAttendancePerYear]);

  console.log('ulta total attendance per year', reversedTotalAttendancePerYear);
  console.log('final settlement wages', finalSettlementData?.wages);
  ...
  return (
    <div className='ml-[80px]'>
      ...
      <div id={`${searchParams.month}/${searchParams.year}`} ref={contentRef}>
        ...
        {finalSettlementData && (
          <table
            className={`border-2 border-black  text-center ml-10 ${
              !reverse ? '' : 'hidden'
            }`}
          >
          ...
          >
            <thead className=' py-8 overflow-auto'>
              <tr className='text-black  '>
                ...
                {reversedTotalAttendancePerYear.map((obj, index) => (
                  <th key={index} className='border-2 border-black'>
                    <div>
                      <div className='border-b-2 border-black p-2'>
                        {getOrdinalNumber(index)} &nbsp; year(
                        {
                          reversedTotalAttendancePerYear[
                            reversedTotalAttendancePerYear.length - index - 1
                          ].year
                        }
                        {/* it should have worked with {obj.year} as reversedTotalAttendancePerYear is storing reverse data 
                        but is is not working so i'm putting this {reversedTotalAttendancePerYear.length - index - 1} */}
                        )
                      </div>
                      <div className='flex flex-1 '>
                        <div className='flex-1 border-r-2 border-black p-1 text-center'>
            ...
            <tbody>
              {months.map((month, index) => (
                <tr key={index} className='border-[1px] border-black'>
                  ...
                  {finalSettlementData.wages[index + 1]
                    .reverse()
                    .map((wage, idx) => (
                      <td
                        key={idx}
